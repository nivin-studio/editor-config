...
class SftpCommand(object):
    ...
    def setup_elements(cls, config):
        if not hasattr(SftpCommand, 'elements'):
            SftpCommand.elements = [0,config.get('email'),'sftp_flags','ssh_key_file','psftp']
...
class SftpThread(HookedThread):
    ...
    def make_connection(self):
        ...
        if self.connection is None:
            ...
            merged_config['remote_time_offset'] = 0
        ...
        if str(self.window_id) + '-' + self.config['local_dir'] not in SftpCommand.identifiers:
            SftpCommand.identifiers[
                str(self.window_id) + '-' + self.config['local_dir']] = self.connection_id
        ...
        if self.config['remote_dir'] not in SftpCommand.remote_roots[self.connection_id]:
            try:
                ...
                SftpCommand.remote_roots[self.connection_id].append(
        ...
        if self.connection_id not in SftpCommand.remote_time_offsets:
            SftpCommand.remote_time_offsets[
                self.connection_id] = self.connection.remote_time_offset
        if self.on_connect:
            self.on_connect()
        self.failed = False
        return True
...
class BrowsePathThread(HookedThread, QuickPanelBrowser):
    ...
    def run(self):
        ...
        if list_dir_thread.failed and list_dir_thread.result in (u'File not found',u'Folder not found',u'Permission denied'):
            ...
            listing.extend(
                [p[0] for p in list_dir_thread.result if not is_dir(p[0]) and p[0] != '.'])
...
class ConfirmOverwriteFileThread(HookedThread, SyncThread):
    ...
    def run(self):
        ...
        for path in self.files:
            remote_path = local_to_remote(
                path, path_map, self.config['remote_encoding'])
            ...
            remote_dir = local_to_remote(
                dir, path_map, self.config['remote_encoding'])
            ...
            if list_dir_thread.failed and list_dir_thread.result in (u'File not found',u'Folder not found'):
                upload_file()
                continue
            ...
            remote_mod_time = remote_dict.get(
                os.path.basename(remote_path), None)
...
class SftpDownloadFileCommand(sublime_plugin.WindowCommand, SftpCommand):
    ...
    def run(self, paths=None):
        ...
        if not config:
            ...
            choices = [['Yes', 'Download the file %s' % basename],['No', 'Do not download the file %s' % basename]]
            ...
            if is_dir(remote_path):
                ...
                if remote_time == 'None':
                    ...
                    if diff == 'same age' and not self.config['sync_same_age']:
                        ...
                        continue
                    operation = 'Upload local "%s" (%s) over remote "%s" [%s vs. %s]' % (self.strip(local_path, dir, 'local'),diff, self.strip(remote_path, remote_dir, 'remote'),local_time, remote_time)
...
class SftpDownloadFolderCommand(sublime_plugin.WindowCommand, SftpCommand):
    ...
    def run(self, paths=None):
        ...
        if os.name != 'nt':
            basename = unicodedata.normalize('NFC', basename)
        choices = [['Yes', 'Download the folder %s' % basename],['No', 'Do not download the folder %s' % basename]]
    ...
    def run(self, paths=None):
        ...
        def open_file():
            self.window.run_command(
                'open_file', {'file': fix_windows_path(file)})
...
class SftpSwitchConfigCommand(sublime_plugin.WindowCommand, SftpCommand):
    ...
    def run(self, paths=None):
        ...
        def add_desc(config):
            ...
            config['desc'] = '%s %s@%s%s %s' % (remote_type, user, host, port, remote_path)
            return config
